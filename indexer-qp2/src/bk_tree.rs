...
pub struct BkTree {
    term:     String,
    children: IntMap<BkTree>
}

pub fn write_to_disk(file_name: &str,
                     tree: &BkTree) {
...
fn write_bk_tree_to_bytes(pos:   usize,
                          bytes: &mut [u8],
                          tree:  &BkTree) -> usize {
...
pub fn insert_bk(tree: &mut BkTree,
                 term: String) -> () {
    ...
    match tree.children.get_mut(dist) {
        None        => { tree.children.insert(dist, new_bk_tree(term)); },
        Some(child) => { insert_bk(child, term); }
    }
}

pub fn query_disk_bk(file_name: &str,
                     term:      &String,
                     thresh:    i64,
