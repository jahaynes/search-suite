...
#[derive(Debug)]
pub struct Posting { pub doc_id:    u32
                   , pub term_freq: u32
                   }
...
#[derive(Debug)]
pub struct TermPost<'a> { pub doc_id:     u32
...
pub fn rank_result(ir:                           &IndexRead,
                   doc_freqs:                    &HashMap<&Term, u32>,
                   TermPost{doc_id, term_freqs}: TermPost) -> Scored {
    ...
    match find_doc(ir, DocId(doc_id)) {
        ...
        Some(doc) => {
            ...
            for (term, term_freq) in term_freqs {
                ...
                let score = idf(num_docs, *doc_freq as f64)
                          * bm25(avg_doc_len, doc.doc_len, term_freq);
...
pub fn _rank_results(out_scored:   &mut Vec<Scored>,
                    ir:           &IndexRead,
  ...
  // Keyed on doc_id
  let mut m: IntMap<Scored> = IntMap::new();
  ...
  for (term, postings) in results {
      for p in postings {
        ...
        match find_doc(ir, DocId(p.doc_id)) {
            ...
            None => panic!("shouldn't happen (missing docid {} from {})", p.doc_id, query_params.base_path),

            Some(doc) => {

                // 'remove' is used instead of 'get' because 'insert' (below) does not overwrite
                let old_score = m.remove(p.doc_id.into()).unwrap_or(Scored { uri: "".to_string()
                                                                        , score: 0.0
                                                                        , term_count: 0
                                                                        });
...
fn bm25(av_doc_len: u32,
        doc_len:    u32,
        term_freq:  u32) -> f64 {
    let top = term_freq as f64 * (1.2 + 1.0);
    let bottom = term_freq as f64
               + 1.2 * (1.0 - 0.75 + 0.75 * (doc_len as f64 / av_doc_len as f64));
    top / bottom
}

fn idf(total_num_docs: u64,
       doc_freq:       f64) -> f64 {
